...
def doBackup ():
    ...
    # get id of moistline
    res1 = _sqlite.select_task_by_field(work_dir + PATHS.DB_SET_D, 'sensInLine_sets_plant', 'idCtrl', 'value')
    idMoistline = res1[0]
    ...
    cmd = 'sudo cp ' +  work_dir + PATHS.DB_SET_D + ' ' + PATHS.DB_BACKUP_D + fileBackup
    os.system(cmd)
...
def killProcess ():
    ...
    _sqlite.update_task_by_query(work_dir + PATHS.DB_SET_D, "VACUUM")
    print(f"- Vacuum {PATHS.DB_SET_D}")
    _sqlite.update_task_by_query(work_dir + PATHS.DB_RTD_D, "VACUUM")
    print(f"- Vacuum {PATHS.DB_RTD_D}")
    _sqlite.update_task_by_query(work_dir + PATHS.DB_LAN_D, "VACUUM")
    print(f"- Vacuum {PATHS.DB_LAN_D}")
...
if __name__ == '__main__': 
    ...
    try:
        ...
        _sqlite.update_task_by_field(work_dir + PATHS.DB_SET_D, 'system_status', 'upgSwVer', 'value', SW_VERSION)
        # !! per sicurezza resetto aggiornamento
        _sqlite.update_task_by_field(work_dir + PATHS.DB_SET_D, 'system_ctrl', 'guiUpdtCtrl', 'value', UCC_IDLE)
        ...
        while True:
            ...
            # #res1 = _sqlite.select_task_by_field(work_dir + PATHS.DB_SET_D, 'system_status', 'updtEnb', 'value')
            # #res = res1[0]
            
            # res1 = _sqlite.select_task_by_query(work_dir + PATHS.DB_SET_D, "SELECT value FROM gui_status WHERE ID IN ('screen', 'screenPc') ORDER BY ID")
            # screen = res1[0]
            # screen = screen['value']
            # screenPc = res1[1]
            ...
            if screen == 'info_Screen' or screenPc == 'info_Screen':
                ...
                if res == 1:
                    print("- Found upgrade ---------------------------------------------------------------------------------")
                    res1 = _sqlite.select_task_by_field(work_dir + PATHS.DB_SET_D, 'system_status', 'guiUpdtSts', 'value')
                    res = res1[0]
                    if res['value'] != USV_UPG_AVAILABLE:
                        _sqlite.update_task_by_field(work_dir + PATHS.DB_SET_D, 'system_status', 'guiUpdtSts', 'value', USV_UPG_AVAILABLE)


                    res1 = _sqlite.select_task_by_field(work_dir + PATHS.DB_SET_D, 'system_ctrl', 'guiUpdtCtrl', 'value')
                    res = res1[0]
                    if res['value'] == UCC_START_UPGRADE:

                        # reset
                        _sqlite.update_task_by_field(work_dir + PATHS.DB_SET_D, 'system_status', 'guiUpdtSts', 'value', USV_IDLE)
                        _sqlite.update_task_by_field(work_dir + PATHS.DB_SET_D, 'system_ctrl', 'guiUpdtCtrl', 'value', UCC_IDLE)
                    ...
                    if res['value'] != USV_NO_NEW_VER:
                        _sqlite.update_task_by_field(work_dir + PATHS.DB_SET_D, 'system_status', 'guiUpdtSts', 'value', USV_NO_NEW_VER)
                else:
                    print("Source directory not found")
                    res1 = _sqlite.select_task_by_field(work_dir + PATHS.DB_SET_D, 'system_status', 'guiUpdtSts', 'value')
                    res = res1[0]
                    if res['value'] != USV_NO_JSON:
                        _sqlite.update_task_by_field(work_dir + PATHS.DB_SET_D, 'system_status', 'guiUpdtSts', 'value', USV_NO_JSON)

            elif screen == 'backup_Screen' or screenPc == 'backup_Screen':
                res1 = _sqlite.select_task_by_field(work_dir + PATHS.DB_SET_D, 'system_ctrl', 'usrCmdBoot', 'value')
                usrCmd = res1[0]
                usrCmd = usrCmd['value']
                if usrCmd == UCBOOT_DO_BACKUP:
                    _sqlite.update_task_by_field(work_dir + PATHS.DB_SET_D, 'system_ctrl', 'usrCmdBoot', 'value', UCBOOT_RESET)
                    backup = True
                    break
